...
#if defined(NDEBUG)
# error "antilitecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x743de1f713cc7e6ee4d5ff38f907b9047f29afe46ca1e6abe0223dcc1b6ca2f7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // antilitecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "antilitecoin Signed Message:\n";
...
{
    // antilitecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // antilitecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // antilitecoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // antilitecoin: 30 Seconds.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // antilitecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    if (pindexNew == pindexBest)
    {
        ...
        static uint256 hashPrevantilitecoinBase;
        UpdatedTransaction(hashPrevantilitecoinBase);
        hashPrevantilitecoinBase = GetTxHash(0);
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // antilitecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x743de1f713cc7e6ee4d5ff38f907b9047f29afe46ca1e6abe0223dcc1b6ca2f7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "fuck off Trump";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041384710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1456999184;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3286438;
        ...
        assert(block.hashMerkleRoot == uint256("0xbc09761bf178b204c544125880d20e7a34012b3deb2ca6bb7f37ad9be8aed5ba"));
...
unsigned char pchMessageStart[4] = { 0xc1, 0xd1, 0xd2, 0xad }; // antilitecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("antilitecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("antilitecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static antilitecoinMiner(CWallet *pwallet)
{
    printf("antilitecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("antilitecoin-miner");
    ...
    try { loop {
        ...
        printf("Running antilitecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("antilitecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&antilitecoinMiner, pwallet));
}
